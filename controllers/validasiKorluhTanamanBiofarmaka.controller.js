...
const combineData = (current, before) => {
    ...
    current['masterIds']?.forEach(nt => {
        current[nt]['triwulanLalu'] = before[nt] ? before[nt]['akhir'] || 0 : 0;

        current[nt]['akhir'] = current[nt]['triwulanLalu'] + current[nt]["luasPenanamanBaru"] - current[nt]["luasPanenHabis"] - current[nt]["luasRusak"];
    });
    return current;
}
...
module.exports = {
    ...
    reqValidation: async (req, res) => {
        ...
        try {
            ...
            const validasiKorluhTanamanBiofarmaka = await ValidasiKorluhTanamanBiofarmaka.findByPk(id);
    ...
    data: async (req, res) => {
        try {
            ...
            current = dataMap(current, triwulan, tahun, kec, validasiKorluhTanamanBiofarmaka);
